...
export async function POST(request: Request) {
  ...
  const booking = process.env.MONGODB_URI
    ? await (async () => { await connectMongo(); return Booking.findById(bookingId).lean() })()
  ...
  if (process.env.MONGODB_URI) {
    const updated = await Booking.findByIdAndUpdate(bookingId, { $set: { status: 'paid', razorpayPaymentId: String(razorpay_payment_id) } }, { new: true }).lean()
